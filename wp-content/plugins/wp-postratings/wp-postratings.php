...
function get_ratings_images($ratings_custom, $ratings_max, $post_rating, $ratings_image, $image_alt, $insert_half) {
	...
	if($ratings_custom) { 
		for($i=1; $i <= $ratings_max; $i++) {
			if($i <= $post_rating) {
				/*$ratings_images .= '<img src="'.plugins_url('/wp-postratings/images/'.$ratings_image.'/rating_'.$i.'_on.'.RATINGS_IMG_EXT).'" alt="'.$image_alt.'" title="'.$image_alt.'" class="post-ratings-image test" />';*/
				$ratings_images .= '<div class="love_item a-vote"></div>';
...
function get_ratings_images_vote($post_id, $ratings_custom, $ratings_max, $post_rating, $ratings_image, $image_alt, $insert_half, $ratings_texts) {
	...
	if($ratings_custom) {
		for($i=1; $i <= $ratings_max; $i++) {
			...
			if($i <= $post_rating) {
				$ratings_images .= '<div class="love_item post-ratings-image" onmouseover="current_rating('.$post_id.', '.$i.', \''.$ratings_text.'\');" onmouseout="ratings_off('.$post_rating.', '.$insert_half.', '.$use_half_rtl.');" onclick="rate_post();" onkeypress="rate_post();" ></div>';    
			} elseif($i == $insert_half) {
				if ($use_half_rtl) {
				...
				$ratings_images .= '<div class="love_item"  onmouseover="current_rating('.$post_id.', '.$i.', \''.$ratings_text.'\');" onmouseout="ratings_off('.$post_rating.', '.$insert_half.', '.$use_half_rtl.');" onclick="rate_post();"></div>';
			}
		}
	} else {
		if ('rtl' == $text_direction && file_exists(WP_PLUGIN_DIR.'/wp-postratings/images/'.$ratings_image.'/rating_half-rtl.'.RATINGS_IMG_EXT)) {
